...
import { config, measurements } from './mock.js';
...
/**
 * Main entry point with mock data
 */
async function main () {
    try {
        const { workbook, indexByPozo, indexByCompuesto } = await buildWorkbook(measurements, config);

        // const buffer = await workbook.xlsx.writeBuffer(); // esto sigue siendo async
        // writeFileSync('output.xlsx', buffer);

        // Guarda el archivo Excel
        await workbook.xlsx.writeFile('output.xlsx');
        console.log('Excel generado: output.xlsx');

        // Muestra estructuras de índice
        console.log('Índice por pozo:', indexByPozo);
        console.log('Índice por compuesto:', indexByCompuesto);
    } catch (error) {
